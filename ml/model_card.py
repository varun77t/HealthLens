...
def build_model_card(
    ...
    *,
    ...
    return {
        ...
        "risk_bands": _risk_band_block(alt_threshold_metrics),
        "limitations": _LIMITATIONS.get(disease, []),
        "artifacts": artifacts or {},
    }


def _risk_band_block(alt_threshold_metrics: dict) -> dict:
    """Bands anchored on this model's measured operating threshold, not a fixed grid."""
    threshold = float(alt_threshold_metrics["threshold"])
    return {
        "note": RISK_BAND_NOTE,
...
def _card_markdown(card: dict) -> list[str]:
    ...
    lines = [
        ...
        *[f"- {l}" for l in card["limitations"]],
        ...
        "",
        df_to_markdown(pd.DataFrame(card["risk_bands"]["bands"]), floatfmt="{:.4f}"),
        "",
        "---",
